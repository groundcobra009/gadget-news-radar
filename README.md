...
27本のフィードを毎朝5時台に24時間分収集し、Claude が推奨度（★1〜★3）を付けたHTMLダイジェストを
...
- **24時間窓は JST 5:00 固定境界**。GitHub の cron は実行時刻を保証せず、実測で6時間半
  遅れたこともある。実行時刻から24時間を引く方式だと窓がずれて穴や重複が出るが、
  固定境界ならどれだけ遅れても対象記事は変わらない（遅れるのは届く時刻だけ）
- **cron の分を `:00` にしない**。毎時0分は混雑して遅れやすい。ただし 05:00 JST より
  前に動かすと窓が1日ずれるので、必ず 05:00 より後に置く
